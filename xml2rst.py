...
def sphinx_conf(folder):
    ...
    f.close()
    #source folder
    source_folder = os.path.abspath(os.path.dirname(conf_file))
    #copy generate.py file
    shutil.copyfile("configs/generate.py",os.path.join(source_folder,"generate.py"))
    return True
...
def init_template(rootpath,vals):
    conf_file = glob.glob(
                        os.path.join(rootpath,vals["DOCS"]) + "/**/conf.py",
                        recursive=True)
    if not conf_file[0]:
        return False
    sphinx_source = os.path.dirname(conf_file[0])
    header_comment = ".. "+vals["PROJECT_NAME"]+" documentation master file"+ \
            "\n  generated by XML2RST based on Sphinx guidelines"
    title = "Welcome to "+vals["PROJECT_NAME"]+" documentation!"
    title_sep = ""
    for c in title:
        title_sep +="="
    if os.path.isfile(os.path.join(sphinx_source,"index.rst")):
        os.remove(os.path.join(sphinx_source,"index.rst"))

    with open(os.path.join(sphinx_source,"_index_tmp"),"w") as f:
...
def wizzard ():
    ...
    vals["EXCLUDED"] = input("Please insert a comma separated list of"
            +"files and folders that you want to exclude (optional)")
    ...
    # create initial RST template
    init_template(rootpath,vals)
